...
The objective of this project is to explore the variables of the hr data set that can help in identifying the factors that led some employees to leave the company. During the investigation these questions will be answered:
...
The data set consists of 14,999 observations and 10 variables. It does not have any variable that is a unique identification of the employee, so let's consider that each record corresponds to one person, without duplication.

```{r}
hr$salary<-ordered(hr$salary,levels=c("low","medium","high"))
```

The level of employee satisfaction will be grouped into three levels: YES, NO and REGULAR.
...
ggplot(aes(as.factor(left)),  data = hr) + 
  ...
  stat_count(aes(y     = ..count..,
                 ...
                 geom  = "text", 
                 vjust = -0.5) +
  scale_x_discrete(labels = c("0" = "Remain", 
                              "1" = "Left")) +
  labs(title = "Sum of Remain and Left", 
...
Left, a variable focus of the analysis, is divided between contractors, that is, who are still in the company and who leave. Of the total 14,999 employees, 3,571 were dismissed and 11,428 were active. A proportion of employees who have left a company for 24% of the data set. 
...
ggplot(aes(as.factor(time_spend_company)), data = hr) + 
  geom_histogram(stat   = "count", 
   ...
   stat_count(aes(y     = ..count..,
                 ...
                 geom  = "text", 
                 vjust = -0.5) +
  labs(title = "Number of employees by year worked", 
       y     = "Number of Employees", 
       x     = "Number of years worked")
```
  
This is a positively skewed distribution, with a reduction in the number of  employees per year as it approaches the right tail. Let's check this trend and confirm that the median value is less than the mean value through the statistical summary of the variable time_spend_company:

```{r}
summary(hr$time_spend_company)
```

The median is 3 years, corresponding to the year that in fact has the largest number of employees, being 6,443 people. The largest volume (75%) of employees in the dataset worked between 2 and 4 years, being the limit of the 3rd quartile in the 4th year.
...
ggplot(aes(as.factor(Work_accident)),  data = hr) + 
  ...
  labs(title = "Suffered an accident at work?", 
       y     = "Total", 
       x     = "work accident")
```
...
ggplot(aes(average_montly_hours),  data = hr) + 
  ...
  labs(title = "Average montly hours worked", 
       y     = "Total", 
       x     = "Montly hours worked") +
  geom_density()
...
ggplot(aes(sales),  data = hr) + 
  ...
  stat_count(aes(y     = ..count..,
                 ...
                 geom  = "text", 
                 vjust = -0.5) +
  labs(title = "Number of employees by sales", 
       y     = "Total", 
       x     = "Sales")
```
...
ggplot(aes(as.factor(number_project)),  data = hr) + 
  ...
  stat_count(aes(y     = ..count..,
                 ...
                 geom  = "text", 
                 vjust = -0.5) +
  labs(title = "Number of projects worked", 
       y     = "Total", 
       x     = "Number of projects worked")
```
...
ggplot(aes(last_evaluation),  data = hr) + 
  ...
  labs(title = "Last evaluation grade", 
       y     = "Number of employees", 
       x     = "Grade")
...
ggplot(aes(satisfaction_level),  data = hr) + 
  ...
  #facet_wrap( ~ left) +
  labs(title = "Employee satisfaction level", 
       y     = "Number of employees", 
...
ggplot(aes(satisfied),  data = hr) + 
  ...
  stat_count(aes(y     = ..count..,
                 ...
                 vjust = -0.5) +
  facet_wrap( ~ left) +
  labs(title = "Satisfaction level bandwidth  - (0=Remain, 1=Left)", 
       y     = "Number of employees", 
       x     = "Satisfied")
```
...
The data structure consists of 14,999 observations, 10 variables imported with the original file. The satisfied variable was added to represent in 3 bands (YES, REGULAR, NO) the satisfaction of the employee.
...
1. Promotion_last_5years: The total of promotions received can generate a dissatisfaction in the employee if he supposes that it was not recognized;
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Create a subset of data with left proportion
prop_left_sales <- data.frame(round(prop.table(table(hr$sales,hr$left), 1)*100,0))

# Change columns names
colnames(prop_left_sales) <- c("sales", "left", "Frequency")

# Pivot left
formattable(cast(prop_left_sales, sales ~ left, value="Frequency"), 
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(prop_left_sales, aes(x     = sales, 
                            y     = Frequency, 
                            fill  = left, 
                            label = sprintf("%1.0f%%", Frequency)))+
  geom_col() +
  geom_text() +
  scale_fill_wsj(name    = "left",
                 breaks  = c("0", "1"),
                 labels  = c("Remain", "Left" )) +
  theme(axis.text.x = element_text(angle=90, hjust=1)) +
  labs(title = "Left by sales",
       y     = "Frequency", 
       x     = "Sales")
...
ggplot(hr, aes(x    =  sales, 
  ...
  labs(title = "Average montly hours worked by sales",
       x     = "Sales",
       y     = "Average montly hours") +
  theme(axis.text.x = element_text(angle=90, hjust=1)) +
  scale_fill_wsj(name   = "left",
                 breaks = c("0", "1"),
                 labels = c("Remain", "Left" ))
...
# Create dataset with average montly hours by sales
media_hora_departamento <- aggregate(hr$average_montly_hours, 
                                     by  = list(hr$sales, hr$left ) ,
                                     FUN = mean)

# Chage columns names
colnames(media_hora_departamento) <- c("sales", "left", "Mean")

# Pivot Left
media_hora_departamento <- cast(media_hora_departamento, sales ~ left, 
                                value = "Mean")

# Chage columns names
colnames(media_hora_departamento) <- c("Sales", 
                                       "Remain", 
                                       "Left")

formattable(media_hora_departamento, 
            list(Left = color_tile("gray", "red")))
...
ggplot(hr, aes(x    =  as.factor(time_spend_company), 
  ...
  labs(title = "Montly hours by worked years",
       x     = "Worked years",
       y     = "Average montly hours") +
  scale_fill_wsj(name   = "left",
                 breaks = c("0", "1"),
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Create dataset with average montly hours by time spend company
...
formattable(horas_trabalhadas_ano, 
            list(Left = color_tile("white", "red")))
...
ggplot(hr, aes(x      =  as.factor(time_spend_company), 
  ...
  labs(title = "Satisfaction level by years worked",
...
ggplot(hr, aes(x      =  as.factor(time_spend_company), 
  ...
  geom_point(alpha = 1/5, 
             position = position_jitter(h=0)) + 
  labs(title = "Last evaluation grade by years worked",
       x     = "Years worked",
       y     = "Last evaluation grade") +
  scale_color_wsj(name   = "left",
                  breaks = c("0", "1"),
                  labels = c("Remain", "Left" ))
...
ggplot(hr, aes(x = average_montly_hours , fill=as.factor(number_project))) + 
  ...
  labs(title = "Average montly hours by number of projects",
       x     = "Average montly hours",
       y     = "Total") +
  scale_fill_wsj(name = "Number of projects") 
...
ggplot(aes(x = sales, fill = as.factor(number_project)), data = hr) + 
  ...
  labs(title = "Number of projects by sales",
       y     = "Number of employees", 
       x     = "Sales")
...
ggplot(aes(x = salary, fill = as.factor(number_project)), data = hr) + 
  ...
  stat_count(aes(y      = ..count..,
                 ...
                 vjust  = 1.5) +
  scale_fill_wsj(name    = "Number of projects") +
  labs(title = "TNumber of projects by salary",
       y     = "Number of employees", 
       x     = "Salary")
...
ggplot(aes(x = as.factor(left) , 
  ...
  labs(title = "Suffered an accident at work?",
       x     = "Left",
       y     = "Total") +
  scale_x_discrete(labels = c("0" = "Remain", "1" = "Left")) +
  scale_fill_wsj(name = "Accident?",
                 breaks = c("0", "1"),
                 labels = c("NO", "YES" ))
``` 
...
```{r echo=FALSE}
# Create graph and correlation for selected numeric variables
...
ggplot(aes(x = satisfaction_level, 
  ...
  geom_smooth(method ='lm') +
  labs(title = "Nível de satisfação e média de horas trabalhadas dos Left",
       x     = "Nível de Satisfação",
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
print("Total de colaboradores medianos Left:")
nrow(hr_func_medianos %>%   filter(left==1))

print("Total de bons colaboradores Left:")
...
colnames(total_satisfacao) <- c("Satisfacao", "Remain", "Left")
...
For starters I selected the variables average_montly_hours and satisfaction_level. The satisfaction_level variable, among all of the data set, was the one that had the most significant correlation value with left, being -0.38. In other charts, average_montly_hours brought relevant information, always demonstrating the existence of a higher monthly average of hours worked for Lefts compared to those hired.

We have an average of 198 hours worked for contractors monthly and 224 hours for Lefts. The maximums recorded were 287 and 310 hours respectively.
...
ggplot(aes(x=number_project, y=last_evaluation, col=satisfied),
       data = subset(hr, left=="Left")) +  	
  geom_point(alpha = 1/5, 
             position = position_jitter(h=0)) + 
  scale_x_continuous(breaks = seq(1,7,1)) +
  labs(title = "Nota na última avaliação por total de projetos dos Lefts",
       x     = "Números de projetos trabalhados" ,
