...
fies <- read.csv('P:/Nanodegree/P04_Explore_Summarize/Explore_Summarize_R/sample_fies.csv', sep = ';', na.strings=c("","NA"))[ ,c("SG_UF", "NO_MUNICIPIO", "VL_MENSALIDADE", "TP_FIANCA", "NU_ANO_EXERCICIO_INSC", "NU_PERCENTUAL_PROUNI", "NU_PERCENT_SOLICITADO_FINANC", "VL_REPASSE", "DT_NASCIMENTO", "ST_DEFICIENCIA", "ST_ENSINO_MEDIO_ESCOLA_PUBLICA", "DS_SEXO", "DS_ESTADO_CIVIL", "DS_RACA_COR", "VL_PERC_FINANCIAMENTO", "QT_SEMESTRE_FINANCIADO", "DS_CURSO")]
names(fies)
...
for (i in 1:17) {
  print (paste(names(fies[i]) , sum(is.na(fies[i])), sep="="))
  next
...
grafico3 <- ggplot(aes(fies$ST_ENSINO_MEDIO_ESCOLA_PUBLICA),  data = fies) + 
  ...
  #scale_x_continuous(limits = c(40, 110), breaks = seq(40,110,10)) +
  labs(title="Vindo de escola p�blica?", 
         y="Total", 
         x= "S: Sim, N: N�o, P: Parcial")
...
```{r}
with(fies, cor.test(VL_PERC_FINANCIAMENTO, IDADE, method = 'pearson'))
```
