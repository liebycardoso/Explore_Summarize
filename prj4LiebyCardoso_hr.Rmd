...
ggplot(hr, aes(x =  sales, y = average_montly_hours, fill=as.factor(left))) + 
  ...
  labs(x="Média mensal de horas trabalhadas",
       y="Departamento") +
  theme(axis.text.x = element_text(angle=90, hjust=1)) +
  scale_fill_brewer(palette="Paired",
                    name="left",
...
ggplot(aes(x = as.factor(left) , fill=as.factor(Work_accident)),
       data = hr) +
  geom_bar() +
  labs(x="Deixou a empresa - 0:NÂO, 1:SIM",
       y="Total") +
...
O primeiro passo é criar um dataset com os dados dos que vamos chamar de bons colaboradores que atendem aos determinados critérios:
...
4. No mês, em média, trabalharam de 250 horas
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(hr_func_bons, aes(x =  as.factor(number_project), y = average_montly_hours, fill = as.factor(left))) + 
  geom_boxplot(outlier.colour = "black", outlier.size = 0.5) + 
...
3. As pessoas que deixaram a empresa trabalharam mais horas por mês que os que ainda estão contratados. Para este grupo, mais projetos significou mais trabalho;
4. Os demitidos foram os únicos que trabalharam em 6 projetos ou mais.

```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(hr, aes(x =  as.factor(time_spend_company), y = last_evaluation, colour = salary)) + 
  geom_point(alpha = 1/5, 
             position = position_jitter(h=0)) + 
  labs(x="Nota na avaliação",
       y="Nível de Satisfação") +
...
hr_func_medianos <- hr %>% filter(last_evaluation < 0.70 & time_spend_company < 4 & number_project < 5)
...
# Reflexão
[Maps with r](http://stackoverflow.com/questions/1260965/developing-geographic-thematic-maps-with-r)
[Kaggle - HR analytcs](https://www.kaggle.com/msjgriffiths/d/ludobenistant/hr-analytics/explore-explain-density-decision-trees)
[](http://www.leg.ufpr.br/~fernandomayer/useR/Rmarkdown-rautu.html)
[](http://jeromyanglim.blogspot.com.br/2012/05/getting-started-with-r-markdown-knitr.html)